...
namespace slamware_ros_sdk {
    ...
    void ServerLaserScanWorker::doPerform()
    {
        ...
        // Aurora device clock (scan.info.timestamp_ns) and ROS system clock are
        // independent.  We estimate their offset every scan so that we can
        // express device timestamps in ROS time accurately.
        //   offset = ros_time_ns - device_time_ns
        //   scan_ros_time = scan.info.timestamp_ns + offset
        // The midpoint of the SDK call window is our best estimate of the ROS
        // wall-clock instant corresponding to the device's "now".
        // -----------------------------------------------------------------------
        int64_t t_before_ns = rclcpp::Clock().now().nanoseconds();
        ...
        int64_t t_after_ns = rclcpp::Clock().now().nanoseconds();

        // Skip duplicate scans using the device-side scan timestamp
        if (scan.info.timestamp_ns == latestLidarStartTimestamp_) {
            return;
        }

        // Compute scan_time (actual one-revolution duration) from consecutive device timestamps
        double dblScanDur = 0.1; // fallback: 10 Hz
        if (latestLidarStartTimestamp_ != 0) {
            double dt = static_cast<double>(scan.info.timestamp_ns - latestLidarStartTimestamp_) * 1e-9;
            if (dt > 0.0 && dt < 1.0) {
                dblScanDur = dt;
            }
        }
        latestLidarStartTimestamp_ = scan.info.timestamp_ns;

        // Update clock offset estimate with exponential moving average (alpha=0.05).
        // The midpoint of the SDK call brackets the device's current time, so
        //   measured_offset  =  midpoint_ros_ns  -  device_scan_ns
        // Over many samples this converges to the true offset even if the scan
        // buffer adds a fixed delay.
        int64_t t_mid_ns = (t_before_ns + t_after_ns) / 2;
        int64_t measured_offset = t_mid_ns - static_cast<int64_t>(scan.info.timestamp_ns);
        if (!clockOffsetReady_) {
            clockOffsetNs_    = measured_offset;
            clockOffsetReady_ = true;
        } else {
            clockOffsetNs_ = static_cast<int64_t>(0.95 * clockOffsetNs_ + 0.05 * measured_offset);
        }

        // Convert device timestamp to ROS wall-clock time
        int64_t scan_ros_ns = static_cast<int64_t>(scan.info.timestamp_ns) + clockOffsetNs_;
        rclcpp::Time scanTime(scan_ros_ns, RCL_ROS_TIME);

        int64_t ros_now_ns = t_after_ns;
        int64_t age_ms     = (ros_now_ns - scan_ros_ns) / 1000000LL;
        RCLCPP_DEBUG(rclcpp::get_logger("server workers"),
            "[scan ts] device_ns=%lu  ros_ns=%ld  offset_ms=%.1f  age_ms=%ld",
            scan.info.timestamp_ns, scan_ros_ns,
            clockOffsetNs_ / 1.0e6, age_ms);
        ...
        // Publish a TF entry stamped at the corrected scan time using the pose
        // that the Aurora SDK associated with this exact scan.  tf2 will find
        // an exact match when it resolves laser_frame -> odom at scanTime,
        // eliminating interpolation error during rotation.
        geometry_msgs::msg::TransformStamped scanPoseTF;
        scanPoseTF.header.stamp    = scanTime;
        scanPoseTF.header.frame_id = srvParams.getParameter<std::string>("odom_frame");
        scanPoseTF.child_frame_id  = srvParams.getParameter<std::string>("robot_frame");
        scanPoseTF.transform.translation.x = poseSE3.translation.x;
        scanPoseTF.transform.translation.y = poseSE3.translation.y;
        scanPoseTF.transform.translation.z = poseSE3.translation.z;
        scanPoseTF.transform.rotation.x    = poseSE3.quaternion.x;
        scanPoseTF.transform.rotation.y    = poseSE3.quaternion.y;
        scanPoseTF.transform.rotation.z    = poseSE3.quaternion.z;
        scanPoseTF.transform.rotation.w    = poseSE3.quaternion.w;
        tfBrdcst->sendTransform(scanPoseTF);

        sensor_msgs::msg::LaserScan msgScan;
        msgScan.header.stamp    = scanTime;
        msgScan.header.frame_id = srvParams.getParameter<std::string>("laser_frame");
    ...
    void ServerEnhancedImagingWorker::processDepthCamera(const std_msgs::msg::Header& header)
    {
        ...
        if (!auroraSDK->enhancedImaging.waitDepthCameraNextFrame(0)) {
            return;
        }
    ...
    void ServerEnhancedImagingWorker::processSemanticSegmentation(const std_msgs::msg::Header& header)
    {
        ...
        // Early return if no frame available
        if (!auroraSDK->enhancedImaging.waitSemanticSegmentationNextFrame(0)) {
            return;
        }
