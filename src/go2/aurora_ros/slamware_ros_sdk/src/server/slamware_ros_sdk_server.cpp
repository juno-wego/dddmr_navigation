...
namespace slamware_ros_sdk {
    ...
    bool SlamwareRosSdkServer::init_(std::string& /*errMsg*/)
    {
        ...
        // init all workers
        {
            ...
            // RobotPoseWorker must run BEFORE OdometryWorker so that the TF
            // published by OdometryWorker always uses the freshest pose from
            // the current loop iteration, not the previous one.
            ...
            if (0 < params_.getParameter<float>("system_status_pub_period"))
            {
            ...
            if (0 < params_.getParameter<float>("stereo_image_pub_period"))
            {
            ...
            if (0 < params_.getParameter<float>("enhanced_imaging_pub_period"))
            {
            ...
            if (0 < params_.getParameter<float>("point_cloud_pub_period"))
            {
